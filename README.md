...
- **📤 내보내기**: 헌금·지출·찬조를 각각 또는 전체(멀티시트) xlsx로 저장
- **📥 불러오기**: xlsx 파일 업로드 → 시트명 자동 감지 후 데이터 추가
- **🗂️ JSON 백업**: 전체 설정 포함 완전 백업

**엑셀 불러오기 조건:**
- 시트명에 `헌금`, `지출`, `찬조` 중 하나 포함
- 첫 데이터 행 앞에 `날짜` 컬럼 포함
